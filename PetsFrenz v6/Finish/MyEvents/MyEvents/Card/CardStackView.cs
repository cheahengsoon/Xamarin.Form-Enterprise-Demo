...
namespace MyEvents.Card
{
    public class CardStackView : ContentView
    {
        ...
        public List<PetPhoto> ItemsSource
        {
            get
            {
                return (List<PetPhoto>)GetValue(ItemsSourceProperty);
            }
            set
            {
        ...
        void Setup()
        {
            ...
            for (int i = 0; i < Math.Min(NumCards, ItemsSource.Count); i++)
            {
                ...
                var card = cards[i];
                card.Name.Text = ItemsSource[itemIndex].petname;
                //card.Location.Text = ItemsSource[itemIndex].Location;
                //card.Description.Text = ItemsSource[itemIndex].Description;
                card.Photo.Source = ImageSource.FromFile(ItemsSource[itemIndex].petimage);
                card.IsVisible = true;
        ...
        void ShowNextCard()
        {
            ...
            if (itemIndex < ItemsSource.Count)
            {
                ...
                // set the data
                topCard.Name.Text = ItemsSource[itemIndex].petname;
                //topCard.Location.Text = ItemsSource[itemIndex].Location;
                //topCard.Description.Text = ItemsSource[itemIndex].Description;
                topCard.Photo.Source = ImageSource.FromFile(ItemsSource[itemIndex].petimage);
